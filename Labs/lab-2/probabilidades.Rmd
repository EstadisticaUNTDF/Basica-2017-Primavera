...
los espacioes de trabajo de R. El espacio de trabajo de la consola y el de 
...
obtendran un error al tejer. Esto puede parecer frustrante al principio, pero
...
materia, que dominen la habilidades para crear reportes de análisis de datos
completamente reproducibles. Abajo hay algunos consejos para lograrlo:

- Trabajá siempre en tu documento R Markdown y no en la consola.
- Teje temprano y seguido, comprobá siempre que tu documento tenga todos lo que
  esperas que tenga.
</div> 
...
plantila para este laboratorio. Recuerda que todo el código y tus respuestas
...
-   Usando `calc_racha`, calculen la longitud de las rachas de `sim_basket`, y
    y guarden los resultados en un data frame llamaod `sim_racha`. Noten que 
    dado que el obejto `sim_racha` es un vector y no una variable en un data
    frame no necestiamos primero seleccionar primero el data frame como hicimos
    antes cuando calculamos la longitud de las rachas para los tiros de Kobe.
...
-   Describe la distribución de la longitud de rachas ¿Cuál es la longitud 
    ...
    de tiro de 45%? ¿De cuanto es la racha más larga? Asegúrate de incluir un
    gráfico en tu respuesta.

-   Si fueses a correr la simulación del tirador independiente una segunda vez,
    ¿Cómo esperarías que su distrución de rachas se compare con la distribución
...
-   ¿Cómo se compara la distribución de longitudes de rachas de Kobe Bryant con
    ...
    esta comparación, ¿Tenés evidencias que el modelo de manos calientes se 
...
Este laboratorio fue adaptado para EBCN del laboratorio realizado por Andrew 
Bray y Mine &Ccedil;etinkaya-Rundel basado en un laboratorio escrito por Mark
Hansen de UCLA Statistics.
</div>
